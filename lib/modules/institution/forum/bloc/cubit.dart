...
class InstitutionForumCubit extends Cubit<InstitutionForumState> {
  ...
  InstitutionForumCubit(this._careerService, this._institutionService,
      this._profileService, this._forumService)
      : super(LoadingState());
  ...
  Future<void> fetchPublications(bool isFiltering, List<String> filters) async {
    try {
      ...
      List<ForumPublication> forumPublications = await this
          ._forumService
          .getForumPublications(programId, 0, profile.userId, filters);
      ...
      List<Commission> listCommissions =
          await this._institutionService.getCommissions(programId);

      if (forumPublications.isNotEmpty) {
        emit(DisplayState(forumPublications, profile, institutionSubjects,
            listCommissions, filters));
      } else {
        emit(ForumPublicationsNotFoundState(forumPublications, profile,
            institutionSubjects, listCommissions, isFiltering));
      }
  ...
  void createNewForumPublicationState() async {
    ...
    emit(CreateForumPublicationState(displayState.institutionSubjects,
        displayState.profile, displayState.listCommissions));
  }
  ...
  void viewDetailForumPublicationState(
      ForumPublication forumPublication) async {
    emit(LoadingState());
    ...
    emit(DisplayForumPublicationDetailState(
        forumPublication, profile, listComment));
  }

  void updateForumPublicationState(ForumPublication forumPublication) async {
    var displayState = this.state as DisplayState;
    emit(UpdateForumPublicationState(displayState.institutionSubjects,
        displayState.profile, forumPublication, displayState.listCommissions));
  }

  void filterForumPublicationsState() async {
    var displayState = this.state as DisplayState;
    emit(FilterForumPublicationsState(displayState.institutionSubjects,
        displayState.profile, displayState.listCommissions));
  }

  void filterForumPublicationsStateFromNotFound() async {
    var notFoundState = this.state as ForumPublicationsNotFoundState;
    emit(FilterForumPublicationsState(notFoundState.institutionSubjects,
        notFoundState.profile, notFoundState.listCommissions));
  }

  void addForumPublication(
      String title, String description, List<String> uploadTags) async {
    emit(LoadingState());
    await this
        ._forumService
        .createForumPublication(title, description, uploadTags);
  ...
  void deleteForumPublication(ForumPublication forumPublication) async {
    ...
    await this
        ._forumService
        .deleteForumPublication(forumPublication.idPublication);
    fetchPublications(false, []);
  }

  void updateForumPublication(String title, String description,
      List<String> uploadTags, String idPublication) async {
    emit(LoadingState());
    await this
        ._forumService
        .updateForumPublication(title, description, uploadTags, idPublication);
    fetchPublications(false, []);
  }
  ...
  void addComment(ForumPublication forumPublication, String userId,
      String content, String idPublication) async {
    emit(LoadingState());
  ...
  void addVotePublication(
    ...
    this
        ._forumService
        .addVotePublication(userId, forumPublication.idPublication);
  }
